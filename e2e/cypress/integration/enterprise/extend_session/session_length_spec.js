    ...
    it('should match help text', () => {
        const helpText = {
            extendSessionLengthWithActivity: {
                false: 'When true, sessions will be automatically extended when the user is active in their MatterFOSS client. Users sessions will only expire if they are not active in their MatterFOSS client for the entire duration of the session lengths defined in the fields below. When false, sessions will not extend with activity in MatterFOSS. User sessions will immediately expire at the end of the session length or idle timeouts defined below. ',
                true: 'When true, sessions will be automatically extended when the user is active in their MatterFOSS client. Users sessions will only expire if they are not active in their MatterFOSS client for the entire duration of the session lengths defined in the fields below. When false, sessions will not extend with activity in MatterFOSS. User sessions will immediately expire at the end of the session length or idle timeouts defined below. ',
            },
            sessionLengthWebInDays: {
                ...
                true: 'Set the number of days from the last activity in MatterFOSS to the expiry of the user’s session when using email and AD/LDAP authentication. After changing this setting, the new session length will take effect after the next time the user enters their credentials.',
            },
            sessionLengthMobileInDays: {
                false: 'The number of days from the last time a user entered their credentials to the expiry of the user\'s session. After changing this setting, the new session length will take effect after the next time the user enters their credentials.',
                true: 'Set the number of days from the last activity in MatterFOSS to the expiry of the user’s session on mobile. After changing this setting, the new session length will take effect after the next time the user enters their credentials.',
            },
            sessionLengthSSOInDays: {
                ...
                true: 'Set the number of days from the last activity in MatterFOSS to the expiry of the user’s session for SSO authentication, such as SAML, GitLab and OAuth 2.0. If the authentication method is SAML or GitLab, the user may automatically be logged back in to MatterFOSS if they are already logged in to SAML or GitLab. After changing this setting, the setting will take effect after the next time the user enters their credentials.',
            },
