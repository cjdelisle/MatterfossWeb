...
describe('components/SystemNotice', () => {
    const baseProps = {
        ...
        dismissedNotices: {},
        isSystemAdmin: false,
        notices: [{name: 'notice1', adminOnly: false, title: 'some title', icon: matterfossIcon, body: 'some body', allowForget: true, show: () => true}],
        serverVersion: '5.1',
    ...
    test('should match snapshot for regular user, admin notice', () => {
        const props = {...baseProps, notices: [{name: 'notice1', adminOnly: true, title: 'some title', icon: matterfossIcon, body: 'some body', allowForget: true, show: () => true}]};
        const wrapper = shallow(<SystemNotice {...props}/>);
        expect(wrapper).toMatchSnapshot();
    });

    test('should match snapshot for regular user, admin and regular notice', () => {
        const props = {...baseProps, notices: [{name: 'notice1', adminOnly: true, title: 'some title', icon: matterfossIcon, body: 'some body', allowForget: true}, {name: 'notice2', adminOnly: false, title: 'some title2', icon: matterfossIcon, body: 'some body2', allowForget: true, show: () => true}]};
        const wrapper = shallow(<SystemNotice {...props}/>);
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test('should match snapshot for admin, admin notice', () => {
        const props = {...baseProps, isSystemAdmin: true, notices: [{name: 'notice1', adminOnly: true, title: 'some title', icon: matterfossIcon, body: 'some body', allowForget: true, show: () => true}]};
        const wrapper = shallow(<SystemNotice {...props}/>);
        expect(wrapper).toMatchSnapshot();
    });
    ...
    test('should match snapshot for show function returning false', () => {
        const props = {...baseProps, notices: [{name: 'notice1', adminOnly: false, title: 'some title', icon: matterfossIcon, body: 'some body', allowForget: true, show: () => false}]};
        const wrapper = shallow(<SystemNotice {...props}/>);
        expect(wrapper).toMatchSnapshot();
    });

    test('should match snapshot for show function returning true', () => {
        const props = {...baseProps, notices: [{name: 'notice1', adminOnly: false, title: 'some title', icon: matterfossIcon, body: 'some body', allowForget: true, show: () => true}]};
        const wrapper = shallow(<SystemNotice {...props}/>);
        expect(wrapper).toMatchSnapshot();
    });

    test('should match snapshot for with allowForget equal false', () => {
        const props = {...baseProps, notices: [{name: 'notice1', adminOnly: false, title: 'some title', icon: matterfossIcon, body: 'some body', allowForget: false, show: () => true}]};
        const wrapper = shallow(<SystemNotice {...props}/>);
        expect(wrapper).toMatchSnapshot();
    });
});
