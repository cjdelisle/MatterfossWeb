...
import {WarnMetricStatus} from 'matterfoss-redux/types/config';
...
export default class WarnMetricAckModal extends React.PureComponent<Props, State> {
    ...
    renderContactUsError = () => {
        ...
        const mailRecipient = 'support-advisor@matterfoss.com';
        const mailSubject = 'MatterFOSS Contact Us request';
        let mailBody = 'MatterFOSS Contact Us request.';
        if (this.props.warnMetricStatus.id === WarnMetricTypes.SYSTEM_WARN_METRIC_NUMBER_OF_ACTIVE_USERS_500) {
            mailBody = 'MatterFOSS Contact Us request.\r\nMy team now has 500 users, and I am considering MatterFOSS Enterprise Edition.';
    ...
    render() {
        ...
        if (this.props.warnMetricStatus.id === WarnMetricTypes.SYSTEM_WARN_METRIC_NUMBER_OF_ACTIVE_USERS_500) {
            headerTitle = (
                <FormattedMessage
                    ...
                    defaultMessage='Scaling with MatterFOSS'
                />
            );
        } else if (this.props.warnMetricStatus.id === WarnMetricTypes.SYSTEM_WARN_METRIC_NUMBER_OF_POSTS_2M) {
            headerTitle = (
        ...
        let descriptionText;
        const learnMoreLink = 'https://matterfoss.com/pl/default-admin-advisory';

        if (this.props.warnMetricStatus.id === WarnMetricTypes.SYSTEM_WARN_METRIC_NUMBER_OF_ACTIVE_USERS_500) {
            descriptionText = (
                <FormattedMessage
                    id='warn_metric_ack_modal.number_of_active_users.description'
                    defaultMessage='MatterFOSS strongly recommends that deployments of over {limit}} users take advantage of features such as user management, server clustering, and performance monitoring. Contact us to learn more and let us know how we can help.'
                    values={{
        ...
        const subText = (
            ...
            >
                <FormattedMessage
                    ...
                    defaultMessage='By clicking Acknowledge, you will be sharing your information with MatterFOSS Inc. {link}'
                    values={{
                        link: (
                            <ErrorLink
                                url={learnMoreLink}
